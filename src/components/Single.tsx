...
const Single:React.FC<IPropsSingle> = ({restaurant}) => {

    const render = ():JSX.Element => {
        if(restaurant !== null){
            return (
                <div className="flex flex-col rounded border shadow-lg p-4">
                    <img src={restaurant?.imageUrl} alt={restaurant.imageUrl} className="w-full h-96 rounded-lg mb-2 object-cover" />
                    <h1 role="heading" className="p-2 text-xl font-black uppercase">{restaurant?.name}</h1>
                ...
                <p className="text-sm">Review Count: {restaurant?.reviewCount}</p>
        ...
        return (
            <div className="rounded border shadow-lg p-4">
                <div role="status" className="mx-auto text-center">
                    <svg aria-hidden="true"
                         className="w-52 h-52 mr-2 text-purple-200 animate-spin fill-purple-900"
